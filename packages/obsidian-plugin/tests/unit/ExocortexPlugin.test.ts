...
describe("ExocortexPlugin", () => {
  ...
  describe("onload", () => {
    it("should initialize all components successfully", async () => {
      ...
      // - 3 exocmd-cache invalidation handlers (RFC c7da0bca Phase 3c-2 — metadata `changed`, vault `delete`, vault `rename` filtered to assetspaces/exocmd) = 17
  ...
  // RFC c7da0bca Phase 3c-2 — deleted the "Issue #3190 — bootstrap-
  // resolved gate around exocmd cache invalidation" describe block.
  // The subject matter (`bindingsCache.clear()` + fast-resolver
  // invalidation gated by `bootstrapResolved`) is gone with the
  // legacy cold-start infrastructure deletion. Per-file invalidation
  // on the lazy loader is exercised by `LazyAssetGraphLoader`'s own
  // unit tests (`forget` + `clearAll` + race tests).
