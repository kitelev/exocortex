...
describe("VaultSettingsRegistry", () => {
  it("contains exactly 25 descriptors (21 boolean / 2 string / 2 stringList)", () => {
    expect(VAULT_SETTINGS_REGISTRY).toHaveLength(25);
    const byType = { boolean: 0, string: 0, stringList: 0 };
    for (const d of VAULT_SETTINGS_REGISTRY) byType[d.datatype]++;
    expect(byType).toEqual({ boolean: 21, string: 2, stringList: 2 });
  });
