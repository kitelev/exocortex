...
export class ExocortexSettingTab extends PluginSettingTab {
  ...
  private renderProfileSections(containerEl: HTMLElement): void {
    ...
    // conflict cache and resolved offline (offline-resolution program). The
    // `exosyncQuarantineRepoUrl` field remains as a deprecated no-op for
    // backward-compat (registry↔graph parity) and is no longer surfaced here.
