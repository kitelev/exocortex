 ...
 */
export type DisplayMatcherHostFunction<THost = unknown> = (
  host: THost,
  metadata: Record<string, unknown>,
) => boolean;
export type DisplayMatcherHostFunctionRegistry<THost = unknown> = Record<
  string,
  DisplayMatcherHostFunction<THost>
>;
...
export class PrintNameRuleService {
   ...
   */
  /**
   * @param host Opaque value handed to every host-function predicate. The engine NEVER
   *   inspects it — it exists so a predicate can reach whatever its platform needs (the
   *   plugin passes Obsidian's `App`). Undefined under a host that registers no functions,
   *   which is the fail-closed default: an unregistered name never participates.
   */
  constructor(
    private readonly vault: VaultMetadataPort,
    private readonly hostFunctions: DisplayMatcherHostFunctionRegistry = {},
    private readonly host: unknown = undefined,
  ) {}
  ...
  private resolveIdentityForms(value: unknown): string[] {
    ...
    for (const id of raw) {
      const fm = this.vault.resolveLinkpathFrontmatter(id);
      if (!fm) continue;
  ...
  private scanVault(): void {
    ...
    // Only frontmatter is ever needed: collectDisplayNameSpec/collectDisplayNamePart take it
    // alone, and a rule's sourceFile comes from the spec's own exo__Asset_uid — never a path.
    for (const fm of this.vault.listFrontmatter()) {
  ...
  private resolveSinglePropertyKey(raw: unknown): string | null {
    ...
    const targetFm = this.vault.resolveLinkpathFrontmatter(target);
    if (targetFm) {
