...
describe("dynamic-command CLI", () => {
  ...
  describe("exec target IRI binding (regression #2996)", () => {
    ...
    it("strips a leading ./ from --target before encoding (canonical contract)", async () => {
      ...
      // No precondition → evaluate not called for sparqlAsk path; we instead
      // assert via the dry-run JSON output (which echoes target unchanged) and
      // by inspecting that --dry-run skipped grounding.
