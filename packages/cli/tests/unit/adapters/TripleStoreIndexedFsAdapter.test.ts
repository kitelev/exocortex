...
function write(rel: string, frontmatter: string | null, body = "Body.\n"): void {
  const full = join(vault, rel);
  mkdirSync(join(full, ".."), { recursive: true });
  const text = frontmatter === null ? body : `---\n${frontmatter}\n---\n${body}`;
  writeFileSync(full, text, "utf-8");
}

/** The loader's product, exactly as `loadVaultTriples` hands it to `apply`. */
async function buildSource(): Promise<{ source: TripleStoreIndexSource; explicit: Triple[] }> {
  const converter = new NoteToRDFConverter(new FileSystemVaultAdapter(vault));
  ...
  return { source: { explicitTriples: triples, zeroTriplePaths }, explicit: triples };
}
...
beforeAll(async () => {
  ...
  write(
    `a/${QUOTED_UID}.md`,
    [`exo__Asset_uid: "${QUOTED_UID}"`, `exo__Asset_label: "Quoted Label"`, `aliases:`, `  - QL`, CLASS_REF].join("\n"),
  );
  ...
  write(`a/${DANGLING_UID}.md`, [`exo__Asset_uid: ${DANGLING_UID}`, `exo__Asset_label: "[[Dangling label link]]"`, CLASS_REF].join("\n"));
  // Label-less asset: the converter synthesises exo:Asset_label = basename.
  write(`a/${LABELLESS_UID}.md`, [`exo__Asset_uid: ${LABELLESS_UID}`, CLASS_REF].join("\n"));
  // Loader-skipped asset (empty optional property → invariant violation → 0 triples).
  write(
    `a/${SKIPPED_UID}.md`,
    [`exo__Asset_uid: ${SKIPPED_UID}`, `exo__Asset_label: Skipped by loader`, `ems__Effort_parent:`, CLASS_REF].join("\n"),
  );
  // Duplicate label in two folders whose names invert creation order.
  write(`z/${DUP_A_UID}.md`, [`exo__Asset_uid: ${DUP_A_UID}`, `exo__Asset_label: Duplicate Label`, CLASS_REF].join("\n"));
  write(`b/${DUP_B_UID}.md`, [`exo__Asset_uid: ${DUP_B_UID}`, `exo__Asset_label: Duplicate Label`, CLASS_REF].join("\n"));
  // Case-different label (no case folding anywhere).
  write(`a/${CASE_UID}.md`, [`exo__Asset_uid: ${CASE_UID}`, `exo__Asset_label: duplicate label`, CLASS_REF].join("\n"));
  // Numeric label (→ xsd:decimal literal) and quoted ISO date-time label (→ xsd:dateTime).
  write(`a/${NUM_UID}.md`, [`exo__Asset_uid: ${NUM_UID}`, `exo__Asset_label: 42`, CLASS_REF].join("\n"));
  write(
    `a/${DATE_UID}.md`,
    [`exo__Asset_uid: ${DATE_UID}`, `exo__Asset_label: "2026-01-02T03:04:05"`, CLASS_REF].join("\n"),
  );
  // Asset that will vanish after the store is built (U7).
  write(`a/${VANISH_UID}.md`, [`exo__Asset_uid: ${VANISH_UID}`, `exo__Asset_label: Vanishing`, CLASS_REF].join("\n"));
  // Prototype with an alias + an instance inheriting it through the prototype chain (U5).
  write(
    `p/${PROTO_UID}.md`,
    [`exo__Asset_uid: ${PROTO_UID}`, `exo__Asset_label: Proto`, `aliases:`, `  - TaskProto`, `exo__Instance_class:`, `  - "[[ems__TaskPrototype]]"`].join("\n"),
  );
  write(
    `p/${INST_UID}.md`,
    [`exo__Asset_uid: ${INST_UID}`, `exo__Asset_label: Inst`, `exo__Asset_prototype: "[[${PROTO_UID}]]"`, CLASS_REF].join("\n"),
  );
  // Prefixed alias key: the converter reads it as exo:Asset_aliases, the scan reads only `aliases` (U10).
  write(
    `a/${PREFIXED_ALIAS_UID}.md`,
    [`exo__Asset_uid: ${PREFIXED_ALIAS_UID}`, `exo__Asset_label: Prefixed alias carrier`, `exo__Asset_aliases:`, `  - PrefixedOnly`, CLASS_REF].join("\n"),
  );
  ...
  write(
    `a/${PIPED_UID}.md`,
    [`exo__Asset_uid: ${PIPED_UID}`, `exo__Asset_label: Piped alias carrier`, `aliases:`, `  - "[[${CLASS_UID}|Task]]"`, CLASS_REF].join("\n"),
  );
  write(
    `a/${BYALIAS_UID}.md`,
    [`exo__Asset_uid: ${BYALIAS_UID}`, `exo__Asset_label: By-alias link carrier`, `aliases:`, `  - "[[QL]]"`, CLASS_REF].join("\n"),
  );
  ...
  write(`a/.hidden-${HIDDEN_UID}.md`, [`exo__Asset_uid: ${HIDDEN_UID}`, `exo__Asset_label: Hidden asset`, CLASS_REF].join("\n"));
...
function fresh(src: TripleStoreIndexSource = source): { scan: NodeFsAdapter; idx: TripleStoreIndexedFsAdapter } {
  return {
...
describe("TripleStoreIndexedFsAdapter — identity with the scan (#4272, req 5ab3d237)", () => {
  it("U1 parity per (key, value): TBox-form label resolves to the class file", async () => {
    expect(await parity({ exo__Asset_label: "ems__Task" })).toEqual([`tbox/${CLASS_UID}.md`]);
    expect(await parity({ exo__Asset_label: "ems__EffortStatusBacklog" })).toEqual([`tbox/${STATUS_UID}.md`]);
  });

  it("U1a parity: aliases — TBox-form alias, plain alias, and a bare [[uid]] wikilink alias", async () => {
    expect(await parity({ aliases: "ems__Task" })).toEqual([`tbox/${CLASS_UID}.md`]);
  ...
  it("U1d parity: an unresolvable wikilink-form label ([[…]] kept as a literal) matches its bracket-stripped text", async () => {
    expect(await parity({ exo__Asset_label: "Dangling label link" })).toEqual([`a/${DANGLING_UID}.md`]);
    expect(await parity({ exo__Asset_label: "[[Dangling label link]]" })).toEqual([`a/${DANGLING_UID}.md`]);
  });

  it("U1b parity: quoted values, numeric and ISO date-time labels, case is NOT folded", async () => {
    expect(await parity({ exo__Asset_label: "Quoted Label" })).toEqual([`a/${QUOTED_UID}.md`]);
    expect(await parity({ exo__Asset_uid: QUOTED_UID })).toEqual([`a/${QUOTED_UID}.md`]);
    expect(await parity({ exo__Asset_label: 42 })).toEqual([`a/${NUM_UID}.md`]);
    expect(await parity({ exo__Asset_label: "42" })).toEqual([`a/${NUM_UID}.md`]);
    expect(await parity({ exo__Asset_label: "2026-01-02T03:04:05" })).toEqual([`a/${DATE_UID}.md`]);
    expect(await parity({ exo__Asset_label: "DUPLICATE LABEL" })).toEqual([]);
    expect(await parity({ exo__Asset_label: "duplicate label" })).toEqual([`a/${CASE_UID}.md`]);
  });

  it("U1c parity: two-key query intersects (one candidate read, not the union); no match is [] on both sides", async () => {
    expect(await parity({ exo__Asset_label: "Duplicate Label", exo__Asset_uid: DUP_B_UID })).toEqual([`b/${DUP_B_UID}.md`]);
    const { idx } = fresh();
    await idx.findFilesByMetadata({ exo__Asset_label: "Duplicate Label", exo__Asset_uid: DUP_B_UID });
    expect(idx.stats.candidateReads).toBe(1);
    expect(await parity({ exo__Asset_label: "Duplicate Label", exo__Asset_uid: QUOTED_UID })).toEqual([]);
    expect(await parity({ exo__Asset_label: "No such label anywhere" })).toEqual([]);
    expect(await parity({ exo__Asset_uid: "00000000-0000-4000-8000-000000000000" })).toEqual([]);
  });

  it("U2 no scan: getMarkdownFiles is never called, no fallback, reads bounded by candidates", async () => {
    const { idx } = fresh();
    ...
    // + the one zero-triple file read while building the index (U8)
    expect(idx.stats.zeroTripleReads).toBe(1);
    expect(meta).toHaveBeenCalledTimes(5 + 1);
    const readPaths = meta.mock.calls.map((c) => String(c[0]));
    expect(readPaths).not.toContain(`a/${QUOTED_UID}.md`);
    expect(readPaths).not.toContain(`a/plain-note.md`);
  });

  it("U3 delegation: unknown key, empty query value and empty query go to the base scan (parity)", async () => {
    const { idx } = fresh();
    const walk = jest.spyOn(idx, "getMarkdownFiles");
    const byOtherKey = await parity({ exo__Instance_class: `[[${CLASS_UID}]]` });
  ...
  it("U4 verification: a basename-synthesised (store-only) label is NOT a match", async () => {
    ...
    const synthesised = explicit.filter(
      (t) => t.subject instanceof IRI && t.subject.value === subject && t.predicate.value === Namespace.EXO.term("Asset_label").value,
    );
    expect(synthesised.map((t) => (t.object as Literal).value)).toEqual([LABELLESS_UID]);
    ...
    expect(await idx.findFilesByMetadata({ exo__Asset_label: LABELLESS_UID })).toEqual([]);
    expect(idx.stats.candidateReads).toBe(1); // the candidate was read and rejected
  });

  it("U5 AC4 explicit-only: an inherited alias (inferred layer, same default graph in the CLI store) is not a candidate", async () => {
    ...
    expect(await store.match(new IRI(vaultPathToIRI(`p/${INST_UID}.md`)), Namespace.EXO.term("Asset_aliases"))).toHaveLength(1);
    // The adapter is fed the EXPLICIT boundary only (what loadVaultTriples reports) …
    const { scan, idx } = fresh();
    expect(await idx.findFilesByMetadata({ aliases: "TaskProto" })).toEqual([`p/${PROTO_UID}.md`]);
    expect(await scan.findFilesByMetadata({ aliases: "TaskProto" })).toEqual([`p/${PROTO_UID}.md`]);
    // … so the instance was never even read: exactly one candidate, the prototype.
    expect(idx.stats.candidateReads).toBe(1);
  });

  it("U6 duplicates: same set as the scan, deterministic path order", async () => {
    ...
    const again = await new TripleStoreIndexedFsAdapter(vault, source).findFilesByMetadata({ exo__Asset_label: "Duplicate Label" });
    expect(again).toEqual(got);
    const reversed: TripleStoreIndexSource = { explicitTriples: [...explicit].reverse(), zeroTriplePaths: source.zeroTriplePaths };
    const viaReversed = await new TripleStoreIndexedFsAdapter(vault, reversed).findFilesByMetadata({ exo__Asset_label: "Duplicate Label" });
    expect(viaReversed).toEqual(got);
  });

  it("U7 findFileByUID parity (bare + quoted) and an unreadable candidate is skipped like the scan", async () => {
    const { scan, idx } = fresh();
    expect(await idx.findFileByUID(CLASS_UID)).toBe(await scan.findFileByUID(CLASS_UID));
    expect(await idx.findFileByUID(QUOTED_UID)).toBe(`a/${QUOTED_UID}.md`);
    expect(await idx.findFileByUID(QUOTED_UID)).toBe(await scan.findFileByUID(QUOTED_UID));
    rmSync(join(vault, `a/${VANISH_UID}.md`));
    try {
      expect(await idx.findFilesByMetadata({ exo__Asset_uid: VANISH_UID })).toEqual([]);
      expect(await scan.findFilesByMetadata({ exo__Asset_uid: VANISH_UID })).toEqual([]);
    } finally {
      write(`a/${VANISH_UID}.md`, [`exo__Asset_uid: ${VANISH_UID}`, `exo__Asset_label: Vanishing`, CLASS_REF].join("\n"));
    }
  });

  it("U8 a loader-skipped asset (invariant violation → 0 triples) is found through zeroTriplePaths — identical to the scan, read once", async () => {
    const { scan, idx } = fresh();
    expect(source.zeroTriplePaths).toEqual([`a/${SKIPPED_UID}.md`]);
    expect(explicit.some((t) => t.subject instanceof IRI && t.subject.value === vaultPathToIRI(`a/${SKIPPED_UID}.md`))).toBe(false);
    expect(await scan.findFilesByMetadata({ exo__Asset_uid: SKIPPED_UID })).toEqual([`a/${SKIPPED_UID}.md`]);
    expect(await idx.findFilesByMetadata({ exo__Asset_uid: SKIPPED_UID })).toEqual([`a/${SKIPPED_UID}.md`]);
    expect(await idx.findFilesByMetadata({ exo__Asset_label: "Skipped by loader" })).toEqual([`a/${SKIPPED_UID}.md`]);
    expect(idx.stats.zeroTripleReads).toBe(1);
    // and not on every lookup: the two lookups above cost two candidate reads, one zero-triple read
    expect(idx.stats.candidateReads).toBe(2);
  });

  it("U9 own writes through the adapter are visible to the next lookup — as they are to the scan", async () => {
    const NEW_UID = "15151515-1515-4151-8151-151515151515";
    const { scan, idx } = fresh();
    const rel = `w/${NEW_UID}.md`;
    const body = (label: string) => `---\nexo__Asset_uid: ${NEW_UID}\nexo__Asset_label: ${label}\n${CLASS_REF}\n---\nBody.\n`;
    try {
      expect(await idx.findFilesByMetadata({ exo__Asset_uid: NEW_UID })).toEqual([]);
      await idx.createFile(rel, body("Fresh label"));
      expect(await idx.findFilesByMetadata({ exo__Asset_uid: NEW_UID })).toEqual([rel]);
      expect(await idx.findFilesByMetadata({ exo__Asset_label: "Fresh label" })).toEqual(await scan.findFilesByMetadata({ exo__Asset_label: "Fresh label" }));
      await idx.updateFile(rel, body("Renamed label"));
      expect(await idx.findFilesByMetadata({ exo__Asset_label: "Fresh label" })).toEqual([]);
      expect(await idx.findFilesByMetadata({ exo__Asset_label: "Renamed label" })).toEqual([rel]);
      await idx.renameFile(rel, `w2/${NEW_UID}.md`);
      expect(await idx.findFilesByMetadata({ exo__Asset_uid: NEW_UID })).toEqual([`w2/${NEW_UID}.md`]);
      await idx.deleteFile(`w2/${NEW_UID}.md`);
      expect(await idx.findFilesByMetadata({ exo__Asset_uid: NEW_UID })).toEqual([]);
      expect(await scan.findFilesByMetadata({ exo__Asset_uid: NEW_UID })).toEqual([]);
    } finally {
  ...
  it("U10 value-form traps: prefixed exo__Asset_aliases, [[uid|alias]], [[Label]]-by-alias, dot-segment path — parity", async () => {
    ...
    expect(await parity({ aliases: "QL" })).toEqual([`a/${BYALIAS_UID}.md`, `a/${QUOTED_UID}.md`]);
    ...
    expect(explicit.some((t) => t.subject instanceof IRI && t.subject.value === vaultPathToIRI(`a/.hidden-${HIDDEN_UID}.md`))).toBe(true);
