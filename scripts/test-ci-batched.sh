...
EXOCORTEX_JEST_ARGS="--config packages/exocortex/jest.config.js --testPathPatterns=(services/(GroundingExecutor(\.[a-zA-Z0-9_-]+)?|AssetConversionService|NoteToRDFConverter\.(issue-2959|rfc-31c1a0be-grounding-ref|asset-filename|issue-3242-labelless-class|filespace-skip|excluded-folders|issue-3468-skip-summary|no-process-env|dual-storage-flag)|FileSpaceDiscovery|ShapeRegistry|ShapeLoader|ShaclLiteValidator(\.issue3488)?|CommandResolver\.(capabilityInheritance|classAncestors))\.test|services/assetspace/AssetSpaceMount\.test|services/sync/(ChangeDetector|SyncEngine(\.filespace|\.direction|\.delete-propagation)?|StructuredMerger|GatedStructuredMerger|MergeShaclGate|diff3|SyncedQuarantineStore|FileWatermarkStore|transportBackoff|secretScan|spaceSpecCore|ParityValidator|assetSemanticCompare|githubRepoReader\.no-buffer)\.test|utilities/base64\.test|application/services/RelationColumnSetResolver(\.property)?\.test|performance/RelationColumnSetResolverPerformance\.test|infrastructure/sparql/executors/(BGPExecutor\.issue-2997|PropertyPathExecutor\.no-process-env)\.test|infrastructure/github/restCommit\.test|integration/dynamic-commands/(grounding-resolve-execute|grounding-phase3b-pipeline|grounding-type-vault-fixture-parity)\.test|domain/models/GroundingFrontmatterParser\.test|domain/profile/TsFloorGuard\.test)\.ts --forceExit"
if [ "$CI" = "true" ]; then
