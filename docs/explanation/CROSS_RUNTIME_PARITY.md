...
| -------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------ | --------------------------------------------------- | --------------------------------- |
| **CLI** (`npx @kitelev/exocortex-cli validate schema --shapes-mode`) | `packages/cli/src/commands/validate-schema.ts`    | `ShapeLoader.loadFromRDFGraph` | `TripleClassHierarchy` (rdfs:subClassOf BFS)        | `domainToAlgebraTriples()` helper |
...
| CLI                                                                                                                                                                                                                                                                                                                                                                               | Plugin                                                                                                                                                                                  |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ShapeLoader.loadFromVaultFS` — reads `exo__Property*.md` files from the filesystem directly. A label-form wikilink (`[[ems__Task]]`) resolves without the target file existing; a bare-UID one (`[[1b20a8f0-…]]`, the RFC-004 strip-canon form) needs the class file on disk, since the UID is resolved through the `uid → label` index the same scan collects (ticket 32d44596) | `ShapeLoader.loadFromRDFGraph` — queries the in-memory RDF triple store for `rdf:type exo:Property` nodes; requires the wikilink targets (class definition files) to exist in the vault |
