...
`packages/core/src/infrastructure/rdf/parsers/TurtleParser.ts` (112 LoC) wraps `NTriplesParser` after a guard on lines 44–48 that throws on any statement containing `;` or `,`. It does not support:
...
3. **Cleanup orphan `packages/core/dist/services/shacl/`** — delete stale compiled SHACL artifacts now that `src/` is gone.
