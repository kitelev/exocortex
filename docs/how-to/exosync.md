...
- **`auth-required`** — HTTP 401, or 403 without rate-limit markers: the
  ...
  same answer a deleted/renamed repo gives, so the engine cannot tell them
  ...
  «`<owner>/<repo>@<branch>` is not reachable … check the token's
  repository allowlist first». Only when the repo IS visible and the base
  commit alone is unknown does the sync report `full-conflict —
  base-mismatch` (a genuinely rewritten / GC'd base).
