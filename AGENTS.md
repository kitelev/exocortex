...
| **L2 — Integration** | Jest + real `GroundingExecutor` | Dynamic-command suites: `packages/exocortex/tests/integration/dynamic-commands/**`. CLI command integration: `packages/cli/tests/integration/commands/**` (currently `apply-profile` + `audit-co-location`). | Dynamic commands through `CommandResolver` / `PreconditionEvaluator` / `GroundingExecutor`. ⚠️ Dynamic-command suites run in CI only if listed in the `EXOCORTEX_JEST_ARGS` allowlist in `scripts/test-ci-batched.sh` (~line 133). Legacy parametrized-catalogue + YAML contract gate retired 2026-05-23 — replaced by RFC v2 byte-diff testing (`aaaa2dea`). |
...
- New `exocmd__Command` or grounding change → L2 suite in `packages/exocortex/tests/integration/dynamic-commands/` **plus** an entry in the `EXOCORTEX_JEST_ARGS` allowlist in `scripts/test-ci-batched.sh` — suites missing from the allowlist do NOT run in CI and rot silently (see «Test Suite Awareness» in `CLAUDE.md`).
